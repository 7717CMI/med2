...
function findTopPerformer(records: DataRecord[], filters: FilterState, currency: 'USD' | 'INR' = 'USD', volumeUnit: string = 'Million Units'): Insight | null {
  ...
  // Note: values in time_series are already in the unit from metadata (e.g., USD Million)
  let valueDisplay = ''
  if (filters.dataType === 'value') {
    if (currency === 'INR') {
      // For INR, use Indian number system (values are already in Million/Cr scale)
      if (topValue >= 100000) {
        valueDisplay = `₹${(topValue / 100000).toFixed(2)} L Cr`
      } else if (topValue >= 1000) {
        valueDisplay = `₹${topValue.toLocaleString('en-IN', { maximumFractionDigits: 1 })} Mn`
      } else {
        valueDisplay = `₹${topValue.toFixed(2)} Mn`
      }
    } else {
      valueDisplay = `${topValue.toLocaleString('en-US', { maximumFractionDigits: 1 })} USD Mn`
    }
  } else {
    valueDisplay = `${topValue.toLocaleString('en-US', { maximumFractionDigits: 1 })} ${volumeUnit}`
  }
